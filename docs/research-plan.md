...
5. **Trigno Mini / Quattro が使えるか**（研究室の在庫、貸出、価格、同一ベースステーションに繋がるか）。
   標準センサは前腕の個々の筋に対して大きすぎ、ECR や FCR を単独で拾えずクロストークが避けられない。
   Mini / Quattro は 4mm 電極で小さく隣接した筋向けに用意されており、これが使えるかどうかで
   力みの定義の選択肢が変わる（`docs/open-questions.md` 2章を参照）
6. **伸筋群と屈筋群を区画レベルで分離できるか**。抵抗をかけた手関節伸展と屈曲をそれぞれ行い、
   背側センサと掌側センサが自分の動作に強く、相手の動作に弱く反応するかを見る。比が3:1程度取れれば
   区画レベルの共収縮を指標に使える。これは10分で確認できるため、5の結果を待たずに実施する
